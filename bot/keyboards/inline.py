"""Inline-клавиатуры — меню, подписка, формат"""
...
def get_confirm_keyboard(lang: str = "ru") -> InlineKeyboardMarkup:
    """Кнопки подтверждения скачивания: Скачать + Отмена"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=t("btn.confirm_download", lang),
            callback_data="confirm_download",
            style="primary",
            icon_custom_emoji_id=E_ID["download"],
        )],
        [InlineKeyboardButton(
            text=t("btn.cancel_download", lang),
