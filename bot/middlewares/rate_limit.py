...
class RateLimitMiddleware(BaseMiddleware):
    """Ограничивает частоту скачиваний — только для текстовых сообщений с Facebook-ссылками"""
    ...
    async def __call__(
        ...
        # проверяем что текст похож на ссылку Facebook
        from bot.utils.helpers import is_facebook_url
        if not is_facebook_url(event.text.strip()):
            return await handler(event, data)
