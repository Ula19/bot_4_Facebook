...
from bot.database.models import Channel, MediaCache, User
...
async def get_cached_media(
    session: AsyncSession, media_url: str, format_key: str
) -> MediaCache | None:
    """Получить кэш по ссылке + формату, если не протух"""
    result = await session.execute(
        select(MediaCache).where(
            MediaCache.media_url == media_url,
            MediaCache.format_key == format_key,
